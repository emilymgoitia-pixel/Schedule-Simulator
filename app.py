...
st.markdown(
    ...
    * { color: var(--primary-text); }
    ...
    .rel-info {
        background: color-mix(in srgb, var(--primary-accent) 8%, white);
        border: 1px solid color-mix(in srgb, var(--primary-accent) 25%, white);
        border-radius: 8px;
        font-size: 12px;
...
def build_gantt(current_df: pd.DataFrame, datahall_rfs: pd.DataFrame | None = None) -> go.Figure:
    ...
    gantt_phase_order = list(reversed(PHASE_ORDER))
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=df["Phase"],
            base=df["Start"],
            customdata=df[["Finish", "DurationDays", "TotalFloat"]] if "TotalFloat" in df.columns
                       else df[["Finish", "DurationDays"]],
            marker=dict(color=colors, opacity=0.9, line=dict(color="#00000022", width=0.8)),
            width=0.32,
            orientation="h",
            hovertemplate=(
                "<b>%{y}</b><br>"
                ...
                + ("Float: %{customdata[2]} days" if "TotalFloat" in df.columns else "")
                + "<extra></extra>"
            ),
            name="Schedule",
        )
    ...
    fig.add_shape(
        ...
        xref="x", yref="paper",
        line=dict(color="#1D264E", dash="dot", width=1), opacity=0.25,
    )
    fig.add_annotation(
        x=today, y=1.02, xref="x", yref="paper",
        text="Today", showarrow=False,
        font=dict(color="#1D264E", size=11), opacity=0.75,
    )

    if datahall_rfs is not None and not datahall_rfs.empty:
        rfs_pts = datahall_rfs.copy()
        rfs_pts["RFSDate"] = pd.to_datetime(rfs_pts["RFSDate"], errors="coerce")
        rfs_pts = rfs_pts[rfs_pts["RFSDate"].notna()]
        enabled_phases_str = df["Phase"].astype(str).tolist()
        if not rfs_pts.empty and "Commissioning" in enabled_phases_str:
            rfs_pts["RFSLabel"] = rfs_pts["RFSDate"].dt.strftime("%m/%d RFS")
            fig.add_trace(
                go.Scatter(
                    x=rfs_pts["RFSDate"],
                    y=["Commissioning"] * len(rfs_pts),
                    mode="markers+text",
                    text=rfs_pts["RFSLabel"],
                    textposition="top center",
                    textfont=dict(color="#1D264E", size=10),
                    marker=dict(size=8, color="#1D264E", line=dict(color="#ffffff", width=1)),
                    customdata=rfs_pts[["DataHall"]],
                    hovertemplate="<b>Data Hall RFS</b><br>Hall: %{customdata[0]}<br>Date: %{x|%d %b %Y}<extra></extra>",
                    name="Data Hall RFS",
                )
            )

    # Legend annotation for critical path colour
    fig.add_annotation(
        x=1, y=-0.06, xref="paper", yref="paper",
        text=(
            "<span style='color:#E85D24'>&#9632;</span> Critical path (0 float) &nbsp;"
            "<span style='color:#6B7AF5'>&#9632;</span> Non-critical"
        ),
        showarrow=False, align="right",
        font=dict(color="#7b86ad", size=10),
    )

    fig.update_layout(
        barmode="overlay",
        height=max(600, len(df) * 42),
        margin=dict(l=8, r=8, t=22, b=32),
    ...
    fig.update_xaxes(
        type="date", showline=True, linewidth=1, linecolor="#d5dcf5",
        gridcolor="#E7EBF4", tickfont=dict(color="#1D264E", size=11),
        title_font=dict(color="#1D264E", size=12),
    )
    fig.update_yaxes(
        showline=False, gridcolor="#E7EBF4",
        tickfont=dict(color="#1D264E", size=12), automargin=True,
    )
    return fig
...
def render_kpi_cards(kpi_rows: pd.DataFrame) -> None:
    first_row_order = [
        "Civil Complete", "Shell Complete", "OFCI Procurement Complete",
        "Fitup Complete", "Project Complete",
    ]
    row1 = (
        kpi_rows.set_index("Milestone")
        .reindex(first_row_order)
        .dropna(subset=["Date Text"], how="all")
        .reset_index()
    )
    ...
    for row_idx, kpi_chunk in enumerate(rows):
        ...
        for col, (_, row) in zip(cols, kpi_chunk.iterrows()):
            with col:
                chip_class = (
                    "kpi-chip highlight"
                    if row["Milestone"] in {"First RFS", "Final RFS"}
                    else "kpi-chip"
                )
                st.markdown(
                    f"""
...
# ---------------------------------------------------------------------------
# OPC file upload (cached by file identity — no re-parsing on every rerun)
# ---------------------------------------------------------------------------
...
with st.sidebar:
    ...
    current_hall_count = (
        int(len(st.session_state.dh_table.index))
        if not st.session_state.dh_table.empty else 0
    )
    hall_count = st.number_input(
        "Number of Data Halls",
        min_value=1, max_value=20,
        value=max(1, current_hall_count), step=1,
        help="Adds/removes Data Halls using DH2/DH3 pattern as a template.",
    )
    ...
    if target_hall_count != current_hall_count:
        ...
        template    = existing_dh if not existing_dh.empty else build_default_datahall_table()
        resized = []
        for i in range(target_hall_count):
            if i < len(existing_dh):
                row = existing_dh.iloc[i].copy()
            else:
                row = template.iloc[2].copy() if len(template) >= 3 \
                      else template.iloc[min(1, len(template) - 1)].copy()
            ...
            resized.append(row)
        st.session_state.dh_table = pd.DataFrame(resized)[["DataHall", "MW", "CxDurationDays", "LagFromPriorDH"]]

    # NTP shift — propagate delta across all phases
    ...
    if new_ntp != prev_ntp:
        ...
        shifted = st.session_state.phases.copy()
        shifted["Start"]  = pd.to_datetime(shifted["Start"],  errors="coerce") + delta
        shifted["Finish"] = pd.to_datetime(shifted["Finish"], errors="coerce") + delta
        st.session_state.phases = shifted
        if new_power == prev_power:
            power_on_date = (prev_power + delta).date()
            new_power = pd.to_datetime(power_on_date)
    ...
    with st.expander("Editing Controls", expanded=False):
        ...
        edited_phases["DurationDays"] = (
            pd.to_numeric(edited_phases["DurationDays"], errors="coerce")
            .fillna(0).clip(lower=0).astype(int)
        )
        edited_phases["Enabled"] = edited_phases["Enabled"].fillna(True).astype(bool)
        st.session_state.phases  = edited_phases[BASE_PHASE_COLS].copy()
...
# ---------------------------------------------------------------------------
# Pre-calculation: apply Power On as Site Power finish
#
# Power On is always the hard constraint for Commissioning start. We enforce
# this through the Site Power → Commissioning FS relationship in the engine.
# Setting Site Power's finish to power_on_ts here is the single authoritative
# place where the Power On date enters the schedule — the engine then
# propagates it forward to Commissioning via the relationship network.
# ---------------------------------------------------------------------------

phases_for_calc = st.session_state.phases.copy()
power_on_ts = pd.to_datetime(power_on_date, errors="coerce")

site_power_mask = phases_for_calc["Phase"] == "Site Power"
if site_power_mask.any() and pd.notna(power_on_ts):
    sp_idx     = phases_for_calc.index[site_power_mask][0]
...
phases_calc, conflicts = recalc_schedule(phases_for_calc, relationships)
phases_calc = add_cadc_rollups(phases_calc)

# Write propagated dates back to session state so the editing table reflects
# the full logic-network output, not just the raw manual entries.
st.session_state.phases = phases_calc[BASE_PHASE_COLS].copy()

# Extend Commissioning bar through Final RFS when Tenant Fitout is disabled
# (so the Gantt communicates the full delivery window).
...
if not dh_results.empty and dh_results["RFSDate"].notna().any():
    ...
    cx_mask          = phases_calc["Phase"] == "Commissioning"
    tfo_mask         = phases_calc["Phase"] == "Tenant Fitout"
    tfo_enabled      = bool(phases_calc.loc[tfo_mask, "Enabled"].any()) if tfo_mask.any() else False
    if cx_mask.any() and not tfo_enabled:
        cx_start = pd.to_datetime(phases_calc.loc[cx_mask, "Start"]).iloc[0]
        if pd.notna(cx_start) and pd.notna(final_rfs) and final_rfs >= cx_start:
            phases_calc.loc[cx_mask, "Finish"]      = final_rfs
            phases_calc.loc[cx_mask, "DurationDays"] = int((final_rfs - cx_start).days)
...
kpi_df["Months Label"] = kpi_df["Date"].apply(
    lambda d: months_from_reference_text(reference_ts, d, reference_label)
)
...
st.plotly_chart(
    build_gantt(phases_calc, dh_results),
    width="stretch",
...
with bottom_left:
    ...
    milestone_panel["IsPast"] = (
        pd.to_datetime(milestone_panel["Date"], errors="coerce")
        < pd.Timestamp.today().normalize()
    )
    milestone_styler = milestone_panel[["Milestone", "Date Text"]].style.apply(
        lambda row: [
            "",
            "color: #9ca3af;" if milestone_panel.loc[row.name, "IsPast"] else "",
        ],
        axis=1,
    )
    st.dataframe(
    ...
    st.caption("Months-after-NTP available in KPI cards above.")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    # Relationship network info
    rel_source = (
        "Loaded from uploaded file"
        if st.session_state.uploaded_file_key and relationships is not DEFAULT_RELATIONSHIPS
        else "Default network"
    )
    st.markdown(
        f'<div class="rel-info">Logic network: {rel_source} — '
        f'{len(relationships)} phase relationships active.</div>',
        unsafe_allow_html=True,
    )
...
with bottom_right:
    ...
    has_value  = dh_display["RFSDate"].notna() | dh_display["MW"].notna()
    dh_display = dh_display[
        (dh_display["DataHall"].astype(str).str.strip() != "") & has_value
    ].copy()
    dh_display["MW Label"] = dh_display["MW"].apply(
        lambda x: "" if pd.isna(x) else f"{x:g} MW"
    )
    ...
    st.dataframe(
        ...
        width="stretch",
        hide_index=True,
        height=40 + 28 * max(1, len(dh_display)),
        row_height=28,
        column_config={
...
# ---------------------------------------------------------------------------
# Calculated schedule expander (includes float / critical path info)
# ---------------------------------------------------------------------------

with st.expander("Calculated Schedule (with CADC rollups)", expanded=False):
    phases_display = phases_calc.copy()
    for col in ["Start", "Finish", "LateStart", "LateFinish"]:
        if col in phases_display.columns:
            phases_display[col] = (
                pd.to_datetime(phases_display[col], errors="coerce")
                .dt.strftime("%d %b %Y")
            )
    # Surface float prominently
    if "TotalFloat" in phases_display.columns:
        phases_display["Critical"] = phases_display["TotalFloat"].apply(
            lambda f: "★" if pd.notna(f) and int(f) == 0 else ""
        )
    st.dataframe(phases_display, width="stretch", hide_index=True)
    st.caption("★ = critical path (zero total float). Late dates computed via CPM backward pass.")
